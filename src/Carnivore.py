...
class Carnivore:
    ...
    def eat(self,herbivores):
        ...
        for herbivore in herbivores:
            if aColideWithB(self.x,self.y,self.radius,herbivore.x,herbivore.y) and herbivore.health>0 and self.hungriness>self.hungrinessThreshold:
                herbivore.health = 0
                self.hasEaten=True
    ...
    def shouldReproduce(self,carnivores):
        for carnivore in carnivores:
            if carnivore != self:
                if aColideWithB(self.x,self.y,self.radius,carnivore.x,carnivore.y):
